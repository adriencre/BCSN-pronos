...
CREATE TABLE IF NOT EXISTS public.matches (
  ...
  reminder_24h_at TIMESTAMPTZ NULL,
  reminder_2h_at TIMESTAMPTZ NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Migrations sécurisées pour ajouter les colonnes de rappels si la table matches existe déjà
ALTER TABLE public.matches ADD COLUMN IF NOT EXISTS reminder_24h_sent BOOLEAN DEFAULT FALSE;
ALTER TABLE public.matches ADD COLUMN IF NOT EXISTS reminder_2h_sent BOOLEAN DEFAULT FALSE;
ALTER TABLE public.matches ADD COLUMN IF NOT EXISTS reminder_24h_at TIMESTAMPTZ NULL;
ALTER TABLE public.matches ADD COLUMN IF NOT EXISTS reminder_2h_at TIMESTAMPTZ NULL;
...
-- Table de Traçabilité des Rappels de Matchs (Anti-doublon)
CREATE TABLE IF NOT EXISTS public.match_reminders (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  match_id BIGINT REFERENCES public.matches(id) ON DELETE CASCADE,
  reminder_type TEXT NOT NULL, -- '24H', '2H', ou 'MANUAL'
  recipients_count INT DEFAULT 0,
  sent_at TIMESTAMPTZ DEFAULT NOW(),
  CONSTRAINT unique_match_reminder_type UNIQUE(match_id, reminder_type)
);
...
DROP POLICY IF EXISTS "Public access match_reminders" ON public.match_reminders;
CREATE POLICY "Public access match_reminders" ON public.match_reminders FOR ALL USING (true) WITH CHECK (true);
